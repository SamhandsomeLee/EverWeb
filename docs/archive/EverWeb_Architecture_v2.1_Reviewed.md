# EverWeb Harness 架构设计 v2.1

> [!WARNING]
> 本文已被 [`EverWeb_Architecture_v2.2_Kimi_First.md`](../architecture/EverWeb_Architecture_v2.2_Kimi_First.md) 取代，仅用于架构演进追溯。新实现、审查和执行计划必须以 v2.2 为准。

> WebRetriever Challenge 2026 · Protocol III
>
> 审查修订版：吸收 v2.0.1 全量审查结论，作为 Phase A 的可实施基线；正式比赛模板发布后必须执行 Contract Reconciliation，未完成前不得将模板相关假设升级为架构不变量。

| 项目 | 内容 |
| --- | --- |
| 文档版本 | v2.1.0 |
| 文档日期 | 2026-07-29 |
| 前序版本 | `WebRetriever_Challenge_Architecture_v2.0.md` |
| 工程名 | EverWeb Harness，简称 EverWeb |
| 目标赛道 | Protocol III：真实网站端到端导航与信息抽取 |
| 主语言 | Python 3.12 |
| 浏览器边界 | Playwright，经官方提供的 CDP URL 连接 |
| 核心模型路线 | Phase A：DeepSeek V4；Phase B：Kimi K2.6 候选；Phase C：数据驱动的混合路由 |
| 运行状态真相源 | Append-only JSONL + 不可变 Artifact + EmergencySnapshot |
| 语义记忆 | EverOS 开发期并行轨道；正式评分路径默认关闭 |
| 文档状态 | 可实施基线；比赛模板相关内容为“待对齐契约” |
...
7. **DeepSeek 与 Kimi 的差值只称为 whole-profile delta；视觉增益用同模型消融测量。**
...
5. **模型路由与视觉能力**：在 Harness 可控后用数据选择，不靠 Benchmark 想象。
...
Phase C 可增加跨厂商 verifier，但确定性 V2 仍是主要防线。
...
### 20.4 ScoringPathProviderManifest
...
```toml
[profile.deepseek]
task_analyzer  = { provider="deepseek", model="deepseek-v4-pro" }
navigator      = { provider="deepseek", model="deepseek-v4-pro" }
navigator_fast = { provider="deepseek", model="deepseek-v4-flash" }
...
[profile.kimi]
task_analyzer  = { provider="moonshot", model="kimi-k2.6" }
navigator      = { provider="moonshot", model="kimi-k2.6" }
navigator_fast = { provider="moonshot", model="kimi-k2.6" }
summarizer     = { provider="moonshot", model="kimi-k2.6" }
extractor      = { provider="moonshot", model="kimi-k2.6" }
verifier       = { provider="moonshot", model="kimi-k2.6" }
vision         = { provider="moonshot", model="kimi-k2.6" }
```

以上为候选配置；正式使用前必须核对比赛允许版本、官方端点和实际返回 model。

### 20.6 DeepSeek 当前事实

截至 2026-07-29，DeepSeek 官方变更日志列出 `deepseek-v4-pro` 与 `deepseek-v4-flash`。模型价格、上下文、限制等易变化信息不写进核心架构，存入版本化 Provider Snapshot。
...
发送后超时标记 `AMBIGUOUS_MODEL_CALL`，Adapter 根据 request ID 和 Provider 能力处理，不盲目重复长链工具调用。
...
## 21. 实验方法：模型、视觉与 Prompt

### 21.1 ExperimentPolicy

Phase A/B 可要求单 profile 单 Provider，用于减少实验混杂；系统架构从一开始支持 mixed。

```python
class ExperimentPolicy(BaseModel):
    single_provider_required: bool
...
### 21.2 Whole-profile A/B

```text
DeepSeek profile vs Kimi profile
```
...
它混合了推理、工具、延迟、上下文、视觉和 Prompt 适配能力。
...
```text
Kimi roles unchanged + NullVision
vs
Kimi roles unchanged + KimiVision
```

更严格：

```text
Navigator/Extractor/Verifier 固定
只切换 VisionPort
```
...
9. Provider 最小请求。
10. configured model 与 returned model。
11. Provider endpoint host。
12. 搜索引擎 denylist。
13. Target HTTP Client 确实禁用。
14. Secret scan。
15. 磁盘、内存、时钟。
16. EmergencyEmitter 可独立生成合法目录。
17. ScorerCompatibilityTest 当前快照。
18. 正式入口参数兼容。
...
| 15 | ModelGateway + Provider Manifest | 中 |
...
| 模型 Profile 混杂 | 错选模型 | whole-profile 与 vision ablation 分开 |
...
6. DeepSeek API 官方变更日志：`https://api-docs.deepseek.com/updates/`
...
*本文档取代 v2.0.1 作为 EverWeb 后续实现基线。任何偏离必须通过 ADR 或 PR 明确记录；正式比赛模板发布后，必须先完成 Contract Reconciliation，再发布冻结版。*
